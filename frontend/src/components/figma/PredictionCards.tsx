...
export function PredictionCards({ predictionData, isLoading, error }: PredictionCardsProps) {
  ...
  const marketTotal = cardsData.predicted_total?.market_total;
  const totalEdge = cardsData.predicted_total?.edge;

  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {/* Win Probability - Real Data from Your Model */}
      <GlassCard glowColor="from-red-500/20 to-rose-500/20" className="p-6 border-red-500/40 shadow-xl shadow-red-500/50">
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-gray-300 text-sm">Win Probability</h3>
            <div className="w-3 h-3 rounded-full bg-red-500/20 border border-red-500/40 shadow-lg animate-pulse"></div>
          </div>
          <div className="text-red-400 drop-shadow-[0_0_10px_rgba(239,68,68,0.3)]">
            <span className="text-5xl font-mono tracking-tight">{favoredProb.toFixed(1)}%</span>
          </div>
          <div className="space-y-1">
            <p className="text-gray-400 text-sm font-mono">{favoredTeam} favored</p>
      ...
      <GlassCard glowColor="from-amber-500/20 to-yellow-500/20" className="p-6 border-amber-500/40 shadow-xl shadow-amber-500/50">
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-gray-300 text-sm">Predicted Spread</h3>
            <div className="w-3 h-3 rounded-full bg-amber-500/20 border border-amber-500/40 shadow-lg animate-pulse"></div>
          </div>
          <div className="text-amber-400 drop-shadow-[0_0_10px_rgba(245,158,11,0.3)]">
            <span className="text-4xl font-mono tracking-tight">{spreadDisplay}</span>
          </div>
          <div className="space-y-1">
            <p className="text-gray-400 text-sm font-mono">Model: {spread.toFixed(1)}</p>
            {cardsData.predicted_spread?.market_spread && (
              <div className="flex justify-between text-xs text-gray-500">
                <span>Market: {cardsData.predicted_spread.market_spread.toFixed(1)}</span>
                {cardsData.predicted_spread?.edge && (
                  <span className="text-green-400">Edge: {cardsData.predicted_spread.edge.toFixed(1)}pt</span>
                )}
              </div>
            )}
          </div>
      ...
      <GlassCard glowColor="from-emerald-500/20 to-green-500/20" className="p-6 border-emerald-500/40 shadow-xl shadow-emerald-500/50">
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-gray-300 text-sm">Predicted Total</h3>
            <div className="w-3 h-3 rounded-full bg-emerald-500/20 border border-emerald-500/40 shadow-lg animate-pulse"></div>
          </div>
          <div className="text-emerald-400 drop-shadow-[0_0_10px_rgba(16,185,129,0.3)]">
            <span className="text-4xl font-mono tracking-tight">{total.toFixed(1)}</span>
          </div>
          <div className="space-y-1">
            <p className="text-gray-400 text-sm font-mono">Model: {total.toFixed(1)}</p>
            {marketTotal && (
              <div className="flex justify-between text-xs text-gray-500">
