...
Requires CollegeFootballData GraphQL API access (Authorization header).
"""
import json
import subprocess
import sys
...
# Reuse the same API key approach as universal_coach_analyzer.py
...
def main():
    """Generate full career profiles for key coaches or a single coach (CLI)."""
    default_coaches = [
    ...
    # CLI override: python generate_full_career_coaches.py Matt Campbell "Iowa State"
    if len(sys.argv) >= 4:
        coaches_to_update = [(sys.argv[1], sys.argv[2], " ".join(sys.argv[3:]))]
    else:
        coaches_to_update = default_coaches
    
    print("="*60)
    print("🏈 FULL CAREER COACH PROFILE GENERATOR")
    print("="*60)
    print(f"Will generate profiles for {len(coaches_to_update)} coach(es)")
